...
def decide(text: str, ledger: dict | None = None, voice_samples: list[str] | None = None, history: list[str] | None = None) -> dict:
    ...
    if voice.get("status") == "ok" and voice.get("score", 100) < 55 and verdict == "READY":
        verdict = "VOICE MISMATCH"
        reasons.append("Draft differs materially from the supplied user writing samples.")
    ...
    return {
        ...
        "voice_fit": voice,
        "note": "Decision support only. Credora does not verify source truth automatically or publish to LinkedIn.",
    }
...
def main() -> int:
    ...
    print(json.dumps(decide(text, ledger, voice_samples, history), indent=2, ensure_ascii=False))
    return 0
